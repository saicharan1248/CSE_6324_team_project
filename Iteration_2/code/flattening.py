...
class Flattening:
    ...
    def _get_source_code_custom_errors(self, compilation_unit):

        source_code = ""
        for error_name in compilation_unit.custom_errors:
            error_node = compilation_unit.custom_errors[error_name]
            args = [f"{arg['name']}:{arg['type']}" for arg in error_node.arguments]
            source_code += f"error {error_name}({','.join(args)});\n"

        source_code += self._get_source_code_custom_errors(compilation_unit)
        source_code += self._get_source_code_top_level(compilation_unit.structures_top_level)
        source_code += self._get_source_code_top_level(compilation_unit.enums_top_level)
        source_code += self._get_source_code_top_level(compilation_unit.variables_top_level)
        source_code += self._get_source_code_top_level(compilation_unit.functions_top_level)
